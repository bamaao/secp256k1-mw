...
void test_bulletproof_inner_product(size_t depth, const secp256k1_ge *geng, const secp256k1_ge *genh) {
    ...
    CHECK(secp256k1_bulletproof_inner_product_prove_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &a, &b, lpt, rpt, depth, secp256k1_bulletproof_ip_test_abgh_callback, (void *) &abgh_data, commit) == 1);
    ...
    CHECK(secp256k1_bulletproof_inner_product_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, geng, genh, &c, &p, &p_offs, NULL, NULL, 0, &a, &b, lpt, rpt, depth, commit) == 0);

    secp256k1_scalar_negate(&p_offs, &p_offs);

    CHECK(secp256k1_bulletproof_inner_product_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, geng, genh, &c, &p, &p_offs, NULL, NULL, 0, &a, &b, lpt, rpt, depth, commit) == 1);
    /* check that verification did not trash anything */
    CHECK(secp256k1_bulletproof_inner_product_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, geng, genh, &c, &p, &p_offs, NULL, NULL, 0, &a, &b, lpt, rpt, depth, commit) == 1);
    /* check that adding a no-op rangeproof skew function doesn't break anything */
    offs_ctx.parity = 0;
    CHECK(secp256k1_bulletproof_inner_product_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, geng, genh, &c, &p, &p_offs, test_bulletproof_offset_callback, (void*)&offs_ctx, 0, &a, &b, lpt, rpt, depth, commit) == 1);
    ...
    offs_ctx.parity = 0;
    CHECK(secp256k1_bulletproof_inner_product_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, geng, genh, &c, &p, &p_offs, test_bulletproof_offset_callback, (void*)&offs_ctx, 1, &a, &b, lpt, rpt, depth, commit) == 1);
...
void test_bulletproof_rangeproof(size_t nbits, size_t expected_size, const secp256k1_ge *geng, const secp256k1_ge *genh) {
    ...
    CHECK(secp256k1_bulletproof_rangeproof_prove_impl(&ctx->ecmult_gen_ctx, &ctx->ecmult_ctx, scratch, &ctx->error_callback, proof, &plen, nbits, v, &blind, &commitp, &secp256k1_ge_const_g2, geng, genh, nonce, NULL, 0) == 1);
    CHECK(plen == expected_size);
    CHECK(secp256k1_bulletproof_rangeproof_verify_impl(&ctx->ecmult_ctx, scratch, &ctx->error_callback, proof, plen, nbits, &commitp, &secp256k1_ge_const_g2, geng, genh, NULL, 0) == 1);
